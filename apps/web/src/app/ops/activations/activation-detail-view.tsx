...
import {
  type AdminActivationFieldActivityLocationsParams,
  type AdminActivationFieldActivitySalesParams,
  ...
  useAdminActivationFieldActivityLocations,
  useAdminActivationFieldActivitySales,
...
export function ActivationDetailView({ activationId }: ActivationDetailViewProps): ReactElement {
  ...
  const detailQuery = useAdminActivationGetActivation(activationId, {
    query: {
  ...
  const fieldActivitySalesParams = useMemo((): AdminActivationFieldActivitySalesParams => {
    const p: AdminActivationFieldActivitySalesParams = { limit: 100 };
  ...
  const fieldActivityLocationsParams = useMemo((): AdminActivationFieldActivityLocationsParams => {
    const p: AdminActivationFieldActivityLocationsParams = { limit: 800 };
    if (fieldActivityFilterUserId.length > 0) {
      p.userId = fieldActivityFilterUserId;
    }
    if (fieldActivityFrom.length > 0) {
  ...
  const fieldSalesQuery = useAdminActivationFieldActivitySales(
    activationId,
  ...
  const fieldLocationsQuery = useAdminActivationFieldActivityLocations(
    activationId,
  ...
  return (
    <div className="w-full space-y-6 pb-16">
      <header className="flex flex-col gap-3 border-b border-border pb-6 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <Link href="/ops/activations" className={cn(calmMutedLinkClass, "inline-block text-sm")}>
            ← All activations
          </Link>
                ...
                <div className="w-full space-y-10">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                      ...
                      pings in time order (oldest → newest); the marker is the latest position. Add
                      promoters to the roster in the Roster tab so their locations appear here.
                    </p>
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      Line items from promoters and merchandizers on this activation&apos;s roster,
                      within the activation window and your filters above. Sales by non-rostered
                      users are hidden.
                    </p>
